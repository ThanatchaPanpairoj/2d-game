...
/**
 * Creates the Thanatcha Panpairoj boss that can spawn. Has a unique shooting pattern in GameComponent.
 * 
 * @author (Thanatcha Panpairoj) 
 * @version (6/6/15)
 */
public class ThanatchaPanpairoj extends EnemyObject implements Boss
{
    /**
     ...
     */
    public ThanatchaPanpairoj(int hp, int fWidth, int fHeight) {
        super("Thanatcha Panpairoj", 100, fWidth, fHeight, (int)(Math.random() * fWidth), (int)(Math.random() * fHeight), new Color((float)Math.random(), (float)Math.random(), (float)Math.random()), 2);
        changeMaxHp(hp);
        changeHp(hp);
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        g2.drawString("Thanatcha", x - 63, y - 10);
        g2.drawString("Panpairoj", x - 63, y + 15);
